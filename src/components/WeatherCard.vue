...
const All_City = City_Weather()
const City = All_City.City[index]

const items = ref([])
const url = ref(`https://${City.ip}`)

function fetchPosts() {
  axios.get(url.value).then((response) => {
    const uniqueDates = [
      ...new Set(response.data.list.map((item) => item.dt_txt.split(' ')[0]))
    ].slice(0, 7)
    items.value = response.data.list.filter((item) =>
...
<template>
  <section class="template">
    <div>
      <h1 class="text_box">
        <strong>สภาพอากาศ {{ City.name }} </strong>
      </h1>
    </div>
            ...
            <div class="card-body p-4">
              ...
              <div class="d-flex flex-column text-center mt-5 mb-4">
                <h6 class="display-4 mb-0 font-weight-bold" style="color: #1c2331">
                  {{ ((i.main.temp - 32) / 1.8 / 6).toFixed(2) }}°C
                </h6>
              ...
              <div class="d-flex align-items-center">
                <div class="flex-grow-1" style="font-size: 1rem">
                  <div>
                    ...
                    <span class="ms-1"> ความเร็วลม: {{ i.wind.speed }} ms/s </span>
                  </div>
                  <div>
                    <i class="fas fa-tint fa-fw" style="color: #868b94"></i>
                    <span class="ms-1"> ระดับความชื้น: {{ i.main.humidity }}% </span>
                  </div>
                  <div>
                    <i class="fas fa-sun fa-fw" style="color: #868b94"></i>
                    <span class="ms-1"> คำอธิบาย : {{ i.weather[0].description }} </span>
                  </div>
                </div>
...
.text_box {
  text-align: center;
  text-shadow: 0px 0px 90px rgba(250, 250, 250, 1);
}
</style>
